...
class Xa_Woocommerce_Membership_Admin {
    ...
    public function enqueue_scripts() {
         ...
         * class.
         */
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/xa-woocommerce-membership-admin.js', array('jquery'), $this->version, false);
    }
    ...
    public function xa_wt_membership_action_links($links) {
        ...
        $plugin_links = array(
            ...
            '<a href="https://wordpress.org/support/plugin/xa-woocommerce-memberships">' . __('Support', 'xa-woocommerce-membership') . '</a>',
            '<a href="https://wordpress.org/support/plugin/xa-woocommerce-memberships/reviews?rate=5#new-post">' . __('Review', 'xa-woocommerce-membership') . '</a>',
        );
    ...
    public function render_user_membership_metabox(WP_Post $post) {
        ...
        //$user_memberships = $user_memberships_obj->get_user_memberships( $user->ID );
        //echo '<pre>';print_r($user_memberships);exit;
        // prepare options
        ...
        //$status_options = apply_filters( 'hforce_edit_user_membership_screen_status_options', $status_options, $user_membership->get_id() );
    ...
    private function output_plan_details_panel($user_membership, $status_options, $plans_obj) {
        ...
        <div class="plan-details">
            ...
            <div class="woocommerce_options_panel">
                ...
                $yy_mm_dd_hint = __('YYYY-MM-DD', 'xa-woocommerce-membership');
    ...
    public function get_membership_plan_options($user_membership = null, $user_id = 0) {
        ...
        if ($user && !user_can($user_id, 'create_users')) {
            ...
            foreach ($membership_plans as $membership_plan) {
                ...
                // Every user can only have 1 membership per plan. check if user already has a membership for this plan.
    ...
    public function get_not_allowed_post_ids_for_user($user_id) {
        ...
        if (!empty($user_memberships)) {
            foreach ($user_memberships as $user_membership) {
                ...
                $plan_ids[] = $plan_id;
//            echo '<pre>';print_r($product_idss);exit;die();
//            if(!empty($product_idss) && is_array($product_idss)){
//                
//                foreach ($product_idss as $id) {
//                    $current_user_only_allowed_products[] = $id;
//                }
//            }elseif(!empty ($product_idss) && is_string($product_idss)){
//                $current_user_only_allowed_products[] = $product_idss;
//            }
            }
        }
...
//                exit;
//                die();
        //$restricted_product_ids = $user_memberships->get_product_id();
        //echo '<pre>';print_r($restricted_product_ids);exit;
        $not_allowed_for_this_user = array();
        $restricted_items = $this->get_restricted_items($plan_ids);
        // echo '<pre>';print_r($restricted_items);exit;die();
        if (!empty($restricted_items)) {
            foreach ($restricted_items as $restricted_item) {
                //echo $restricted_item;
                //if (!in_array($restricted_item, $current_user_only_allowed_products)) {
                $not_allowed_for_this_user[] = $restricted_item;
                //}
            }
        }
        //echo '<pre>';print_r($not_allowed_for_this_user);exit;
