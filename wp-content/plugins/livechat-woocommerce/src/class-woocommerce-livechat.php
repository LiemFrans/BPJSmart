...
class WooCommerce_LiveChat implements WooCommerce_LiveChat_Interface
{
	...
	const LC_S_DISABLE_MOBILE_KEY    = 'wc-lc_disableMobile';
	const LC_S_DISABLE_GUESTS_KEY    = 'wc-lc_disableGuest';
	...
	public function render_script() {
		...
		if (property_exists($currentUserData, 'ID')) {
			$check_logged = true;
		}
	...
	private function get_custom_livechat_data($escapeProductTitle = true) {
		...
		if ( array_key_exists( self::LC_S_PRODUCTS_KEY, $settings ) && 1 == $settings[self::LC_S_PRODUCTS_KEY] ) {
			...
			foreach ($this->get_products_data($escapeProductTitle, (array_key_exists( self::LC_S_PRODUCST_COUNTS_KEY, $settings ) && 1 == $settings[self::LC_S_PRODUCST_COUNTS_KEY])) as $product) {
				$data[$product['title']] = $product['url'];
			}
		}
		if ( array_key_exists( self::LC_S_PRODUCST_COUNTS_KEY, $settings ) && 1 == $settings[self::LC_S_PRODUCST_COUNTS_KEY] ) {
			if ( ( $products_count = WC()->cart->get_cart_contents_count() ) > 0 ) {
				$data['Total count'] = (string) $products_count;
			}
		}
	...
	private function get_integration_settings() {
		...
		if ( array_key_exists( self::LC_S_DISABLE_MOBILE_KEY, $settings ) ) {
			$data['disableMobile'] = $settings[self::LC_S_DISABLE_MOBILE_KEY];
		}

		if ( array_key_exists( self::LC_S_DISABLE_GUESTS_KEY, $settings ) ) {
			$data['disableGuests'] = $settings[self::LC_S_DISABLE_GUESTS_KEY];
		}
	 ...
	 * @return array
	 */
	private function get_products_data($escapeTitle = true, $count = true) {
		$res = array();
		foreach ( WC()->cart->cart_contents as $product ) {
			if ( array_key_exists( 'data', $product ) ) {
				$wc_product = wc_get_product( $product['data'] );
				$qty = ($product['quantity'] > 1 && $count) ? $product['quantity'].'x ' : '';
